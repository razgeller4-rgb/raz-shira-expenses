...
> ## ⚠️ עדכון 2026-09-10 22:40 — התחזית התממשה
>
> המסמך הזה חזה: *"המופע עלול להפוך ללא-מגיב"*. **זה קרה באותו ערב, 11 שעות אחרי שנכתב.**
> המסד הפסיק להגיב לחלוטין — כל בקשה שנגעה ב-Postgres נתקעה ב-12 שניות, רז ננעל
> מחוץ לפרודקשן ולדמו כאחד, ואף מסלול כניסה לא יכול היה לעבוד.
>
> **אפשרות A מומשה ונפרסה** (`b063def` דמו · `cfebcde` פרודקשן) — בתוספת timeout על כל
> הקריאות ומוצא חירום לנתונים מקומיים. ר' `RESEARCH_LOG_AND_LESSONS.md` F-15.
>
> **הלקח:** המסמך זיהה נכון גם את השורש וגם את הסיכון, אבל התיקון נדחה ל"אחר כך".
> ממצא שמוגדר כ"עלול להפיל את המערכת" צריך לוח זמנים, לא תור.
