...
console.log("\n== forward-looking card commitment (U-6) ==");
const s2 = api.getCashOutSummary(sheet);
console.log(`  cardDue (this month only):     ${n(s2.cardDue)}`);
console.log(`  pendingCards (all not-yet-collected): ${n(s2.pendingCards)}  next charge ${s2.nextChargeIso || "—"}`);
console.log(`  byCard:`, JSON.stringify(s2.byCard));
console.log(`  afterCardsBalance:             ${n(s2.afterCardsBalance)}`);
